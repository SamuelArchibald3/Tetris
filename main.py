...
def draw_window(surface, grid, score=0, last_score = 0,level = 0):
    ...
    surface.blit(label, (sx + 45, sy - 30))
    for i in range(len(grid)):
        for j in range(len(grid[i])):
...
def main(win,level):
    ...
    while run:
        ...
        if fall_time / 1000 >= fall_frames/frame_rate:
            fall_time = 0
            current_piece.y += 1
        ...
        if change_piece:
            ...
            clears = clear_rows(grid, locked_positions)
        ...
        label = font.render('FPS: ' + str(int(clock.get_fps())), 1, (255, 255, 255))
...
level = input("enter the level(speed) you'd like to start on (0-29):")
while True:
    try:
        level = int(level)
        if 0 <= level <= 29:
            ...
            level = input("enter the level(speed) you'd like to start on (1-29):")
    except:
        level = input("Please enter an integer: ")
